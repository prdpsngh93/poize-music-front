    ...
    <nav className={`w-full z-50 font-poppins font-normal lg:font-bold text-sm uppercase ${
      ...
      <div className="max-w-[1620px] w-full mx-auto px-4 md:px-9 lg:px-12">
        <div className={`flex border-b-[0.5px] ${
          ...
          <div className="hidden md:flex space-x-4 items-center">
            ...
            <Link href="/cart" className="hover:underline">Cart(0)</Link>
      ...
      {isOpen && (
        <div className="md:hidden bg-white/90 px-6 py-4 space-y-4">
          {navItems.map((item) => (
            <Link key={item} href="#" className="block border-b border-gray-300 py-2 text-black hover:underline">
              {item}
            </Link>
          ))}
          <Link href="/search" className="block border-b border-gray-300 py-2 text-black hover:underline">Search</Link>
          {isLoggedIn ? (
            <button onClick={handleLogout} className="block border-b border-gray-300 py-2 text-black hover:underline">
              Logout
            </button>
          ) : (
            <Link href="/login" className="block border-b border-gray-300 py-2 text-black hover:underline">
              Login
            </Link>
          )}
          <Link href="/cart" className="block border-b border-gray-300 py-2 text-black hover:underline">Cart(0)</Link>
        </div>
      )}
    </nav>
  );
